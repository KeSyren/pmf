...
#         self.WI_user = np.eye(num_feature, dtype='float16')
        self.WI_user = np.eye(num_feature)
        ...
        self.WI_item = np.eye(num_feature)
    ...
    def estimate(self, iterations=100, tolerance=1e-5):
        ...
        for iteration in xrange(iterations):
            ...
            self._get_distribute_params()

            # update item & user_features
            self.ret_mu_Sigma_item = self._udpate_item_features()
    ...
    def _get_distribute_params(self):
        # same as _get_distribute_params
        N = self.num_user
    ...
    def _udpate_item_features(self):
        ...
        for item_id in xrange(self.num_item):
            ...
            item_parameters = (mean, covar)
            ret.append(item_parameters)
        return ret
