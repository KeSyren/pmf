...
class ThompsonSamplingCluster():
    ...
    def _estimate(self, train, tolerance=1e-5):
        ...
        for id in xrange(len(train)):
            ...
            self.mu_cluster, self.Sigma_cluster = self._get_distribute_params(M, n, self.delta)
            # sample p
            # lam = cholesky(self.Sigma_user)
            lam = cholesky(self.Sigma_cluster)
            lam = lam.T
            ...
            sample_cluster_feature = self.mu_cluster + np.dot(lam, rand.randn(self.num_feature, 1))
            sample_cluster_feature_T = np.reshape(sample_cluster_feature, (self.num_feature, ))
            ...
            for item_id in xrange(len(self.item_features)):
                ...
                # estimated_reward = np.dot(sample_user_feature, sample_item_feature_T)
                estimated_reward = np.dot(sample_cluster_feature_T, sample_item_feature)
...
#     ratings = np.array(ratings)
    ...
    ts_model = ThompsonSamplingCluster(rating_matrix, user_feature, num_feature, mu_Sigma_items, delta, lamda_p, ratings) 
    ts_model._init_parameters_cluster_user(20, num_user)
    
    result = ts_model._estimate(ratings)
    thompson_sampling_file = open('../result/ret_cluster_thompson_sampling', 'w')
    for item in result:
